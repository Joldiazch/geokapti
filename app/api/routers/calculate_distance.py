...
# Celery
from app.api.celery import calculate_distance_task
# Utils
...
async def calculate_distance(locations_ids: List[int], user: User = Depends(get_current_user)) -> Location:
    """
    Description:
    ...
    """
    try:
        task = calculate_distance_task.delay(locations_ids)
        return JSONResponse(
